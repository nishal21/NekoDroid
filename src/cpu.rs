...
impl Cpu {
    ...
    pub fn reset(&mut self) {
        ...
        self.halted = false;
        // Note: RAM is not cleared on reset (like real hardware)
    }

    /// Fetches the next instruction word from memory at the current PC.
    /// In ARM mode, fetches 4 bytes; in Thumb mode, fetches 2 bytes.
...
mod tests {
    ...
    fn test_cpsr_flags() {
        ...
        rf.set_flag_n(false);
        assert!(!rf.flag_n());
        assert!(rf.flag_z()); // others unchanged
    }

    #[test]
    fn test_thumb_mode() {
        let mut rf = RegisterFile::new();
        assert!(!rf.is_thumb()); // default: ARM mode
        rf.set_thumb(true);
        assert!(rf.is_thumb());
        rf.set_thumb(false);
        assert!(!rf.is_thumb());
    ...
    fn test_cpu_fetch_arm() {
        ...
        // Write an ARM instruction at address 0
    ...
    fn test_cpu_fetch_thumb() {
        let mut cpu = Cpu::new(1024);
        cpu.mmu.write_u16(0, 0x2001); // MOVS R0, #1
        cpu.regs.set_pc(0);
    ...
    fn test_cpu_load_program() {
        let mut cpu = Cpu::new(1024);
        let prog = [0x01, 0x10, 0xA0, 0xE3]; // MOV R1, #1 (ARM encoding LE)
